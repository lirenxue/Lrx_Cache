...
void testHotDataAccess() {
    ...
    LrxCache::KLruCache<int, std::string> lru(CAPACITY);
    LrxCache::KLfuCache<int, std::string> lfu(CAPACITY);
    LrxCache::KArcCache<int, std::string> arc(CAPACITY);

    std::random_device rd;
    std::mt19937 gen(rd());
    
    std::array<LrxCache::KICachePolicy<int, std::string>*, 3> caches = {&lru, &lfu, &arc};
